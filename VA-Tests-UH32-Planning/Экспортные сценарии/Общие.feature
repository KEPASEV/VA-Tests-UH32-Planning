...
//	Тогда открылось окно '[TheListName]'
	...
	Если элемент формы с именем "CompareType" стал равен '2' тогда
...
Сценарий: Я для реквизита 'TheProperty' устанавливаю значение 'TheValue'
	...
	Иначе
		И из выпадающего списка с именем '[TheProperty]' я выбираю по строке "[TheValue]"
...
Сценарий: Я удаляю текущий элемент в окне 'TheWindowName' очищая 'TheFieldName1' 'TheFieldName2' 'TheFieldName3'
	...
	Если открылось окно '* (Вид цены)' тогда
		И я нажимаю на кнопку с именем 'РазрешитьРедактированиеРеквизитовОбъекта'
		Если открылось окно 'Разблокирование реквизитов' тогда
			И я нажимаю на кнопку с именем 'РазрешитьРедактирование'
		Тогда в поле с именем 'Идентификатор' я ввожу значение переменной 'УИД'
