...
Сценарий: Я создаю тип цен номенклатуры с именем 'TheName'
	* Открываем список
		И я открываю основную форму списка справочника 'ТипыЦенНоменклатуры'
	...
	* Удаляем элемент
		...
		И Пока в таблице 'Список' количество строк 'больше' 0 Тогда
			И в таблице 'Список' я выбираю текущую строку
			И Я удаляю текущий элемент в окне "[TheName] (Тип цены номенклатуры)" очищая

	* Создаем элементм
		И я нажимаю на кнопку с именем 'ФормаСоздать'
		И в поле с именем 'Наименование' я ввожу текст '[TheName]'
		Если флаг 'Цена включает НДС' равен 'Нет' тогда
...
Сценарий: Я создаю Приказ о назначении закупочной комиссии для организации 'TheBusinessUnitName' 
		...
		И Я в списке "Список" по полю "Наименование" ищу и выбираю элемент "[TheBusinessUnitName]" "По точному совпадению" 	
