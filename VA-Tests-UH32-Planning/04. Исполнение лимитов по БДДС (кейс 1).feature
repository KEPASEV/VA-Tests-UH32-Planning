...
Сценарий: 04.01.2 Создаем Заявку на оплату (по договору без графика расчетов)				
	...
	* Проверить движения
		...
		И область "R6C2:R8C28" табличного документа 'ОтчетТабличныйДокумент' равна по шаблону:
			...
			| '04.03.2024 0:00:00' | 'Бюджет движения денежных средств' | 'Заявка на оплату *; ВА - Поставщик1; Оплата поставщику' | ''          | 'ВА - ЦФО 1 (АХО); ВА - ООО Тест' | 'ВА - Статья 1 (ВА - Здания;  - ;  - ;  - ;  - ;  - ) Расход' | ''                       | ''                       | ''                   | 'ВА - ООО Тест; ВА - Поставщик1; 04.01.2 от ; 04.01.2 от ' | 'Расход'        | 'RUB'    | 'RUB'                  | '3 000,00'  | ''           | '3 000,00'  | ''                | ''                     | ''                | '*'                | ''                | 'Нет'                   | '1,0000'            | '1'                      | '1,0000'             | '1'                       | ''                        |
			| '04.03.2024 0:00:00' | 'Бюджет движения денежных средств' | 'Резервирование планов и лимитов *'                      | ''          | 'ВА - ЦФО 1 (АХО); ВА - ООО Тест' | 'ВА - Статья 1 (ВА - Здания;  - ;  - ;  - ;  - ;  - ) Расход' | ''                       | ''                       | ''                   | 'ВА - ООО Тест; ВА - Поставщик1; 04.01.2 от '              | 'Расход'        | 'RUB'    | 'RUB'                  | '-3 000,00' | ''           | '-3 000,00' | ''                | ''                     | ''                | '*'                | ''                | 'Нет'                   | '1,0000'            | '1'                      | '1,0000'             | '1'                       | ''                        |
...
Сценарий: 04.03.2 Создаем Заявку на корректировку лимита
	...
	* Проверяем средства к обеспечению в Отчете по лимитам
	...
	//	И в табличном документе 'ОтчетТабличныйДокумент' ячейка с адресом 'R86C4' равна '$$КОбеспечению$$'
		Дано Табличный документ 'ОтчетТабличныйДокумент' равен макету "Examples\ОтчетПоЛимитамКОбеспечениюПроект" по шаблону
		И Я закрываю окно 'Отчет по лимитам (период в строках)'
		Тогда открылось окно '1С:Предприятие'
		И я нажимаю на кнопку 'Нет'							
