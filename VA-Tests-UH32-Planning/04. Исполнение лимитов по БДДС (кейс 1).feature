...
Сценарий: 04.01.1 Создаем Резервирование планов и лимитов по ЦФО 1 (АХО) на 1 квартал 2024 год
	...
	* Подтвердить резервирование 
		...
		И я запоминаю заголовок текущего окна как "ЗаголовокОкнаРезерв"
	...
	* Проверить движения
		...
		И таблица "ОперацииБюджетов" стала равной:
			...
			| '1' | '01.02.2024 0:00:00' | 'Бюджет движения денежных средств' | 'Расход'        | '1'                       | '$ЗаголовокОкнаРезерв$' | '100 000,00' | 'ВА - ООО Тест'       | ''                | ''          | '1,0000'            | ''                       | ''                | 'ВА - ЦФО 1 (АХО); ВА - ООО Тест'                | ''                | ''                   | ''           | '100 000,00' | 'RUB'    | ''                     | ''                       | 'RUB'                  | '1'                      | 'Нет'                   | '1,0000'             | 'ВА - Статья 1 (ВА - Автотранспорт;  - ;  - ;  - ;  - ;  - ) Расход' | ''                        |
			| '2' | '01.03.2024 0:00:00' | 'Бюджет движения денежных средств' | 'Расход'        | '1'                       | '$ЗаголовокОкнаРезерв$' | '200 000,00' | 'ВА - ООО Тест'       | ''                | ''          | '1,0000'            | ''                       | ''                | 'ВА - ЦФО 1 (АХО); ВА - ООО Тест'                | ''                | ''                   | ''           | '200 000,00' | 'RUB'    | ''                     | ''                       | 'RUB'                  | '1'                      | 'Нет'                   | '1,0000'             | 'ВА - Статья 1 (ВА - Здания;  - ;  - ;  - ;  - ;  - ) Расход'        | ''                        |
			| '3' | '01.03.2024 0:00:00' | 'Бюджет движения денежных средств' | 'Расход'        | '1'                       | '$ЗаголовокОкнаРезерв$' | '300 000,00' | 'ВА - ООО Тест'       | ''                | ''          | '1,0000'            | ''                       | ''                | 'ВА - ЦФО 1 (АХО); ВА - Проект 1; ВА - ООО Тест' | ''                | ''                   | ''           | '300 000,00' | 'RUB'    | ''                     | ''                       | 'RUB'                  | '1'                      | 'Нет'                   | '1,0000'             | 'ВА - Статья 4 Расход'                                               | ''                        |
		
	* Закрыть все окна
		И я закрываю все окна клиентского приложения											
	
Сценарий: 04.01.2 Проверяем зарезервированные средства в Отчете по лимитам
...
Сценарий: 04.01.3 Создаем Заявку на оплату на основании счета (по договору без графика расчетов)				
	...
	* Проверить движения
		...
		И таблица "ОперацииБюджетов" стала равной:
			...
			| '2' | '$ЖелательнаяДатаПлатежа$ 0:00:00' | 'Бюджет движения денежных средств' | 'Расход'        | '1'                       | '$ЗаголовокОкнаРезерв$' | '-3 000,00' | 'ВА - ООО Тест; ВА - Поставщик1; Без договора'               | ''                | ''          | '1,0000'            | ''                       | ''                | 'ВА - ЦФО 1 (АХО); ВА - ООО Тест' | ''                | ''                   | ''           | '-3 000,00' | 'RUB'    | ''                     | ''                       | 'RUB'                  | '1'                      | 'Нет'                   | '1,0000'             | 'ВА - Статья 1 (ВА - Здания;  - ;  - ;  - ;  - ;  - ) Расход' | ''                        |

	* Закрыть все окна
		И я закрываю все окна клиентского приложения											
				
Сценарий: 04.01.4 Проверяем заявленные средства в Отчете по лимитам
	* Сформировать отчет по БДДС за 2024	
		И Я формирую Отчет по лимитам по виду бюджета 'БДДС' за период с "01.01.2024" по  "31.12.2024" в валюте 'RUB'	
		И Я добавляю отбор в Отчете по лимитам 'ЦФО' 'Равно' 'ВА - ЦФО 1 (АХО)'
        И я нажимаю на кнопку с именем 'СформироватьОтчет'
...
Сценарий: 04.01.5 Создаем Платежный документ
...
Сценарий: 04.01.6 Отражаем фактические данные об оплате	 		 		  		 		 		 	 
...
Сценарий: 04.01.7 Проверяем факт оплаты в Отчете по лимитам
	* Сформировать отчет по БДДС за 2024
		И Я формирую Отчет по лимитам по виду бюджета 'БДДС' за период с "01.01.2024" по  "31.12.2024" в валюте 'RUB'	
		И Я добавляю отбор в Отчете по лимитам 'ЦФО' 'Равно' 'ВА - ЦФО 1 (АХО)'
        И я нажимаю на кнопку с именем 'СформироватьОтчет'
...
Сценарий: 04.02.1 Создаем Заявку на оплату с превышением резерва
	...
	* Сохранить документ
		...
		И я запоминаю заголовок текущего окна как "ЗаголовокОкнаЗнО2"
		И я запоминаю значение поля с именем 'Номер' как 'НомерЗнО2'		
		И я запоминаю значение поля с именем 'СуммаДокумента' как 'СуммаЗнО2'
		И я запоминаю значение поля с именем 'ЖелательнаяДатаПлатежа' как 'ЖелательнаяДатаПлатежа2'
...
Сценарий: 04.02.2 Создаем Заявку на корректировку резерва
	...
	* Создать Заявку на корректировку
		Когда открылось окно '$ЗаголовокОкнаЗнО2$'
	...
	* Проверить заполнение
		...
		И элемент формы с именем "ДокументОснование" стал равен '$ЗаголовокОкнаЗнО2$'
		...
		И таблица "Планы" стала равной:
			...
			| '1' | 'ВА - Статья 1'  | ''         | 'ВА - Здания' | ''           | '<Не используется>' | '<Не используется>' | '<Не используется>' | '<Не используется>' | '<Не используется>' | ''      | '$ЗаголовокОкнаРезерв$' | '3 000,00' |
	...
	* Провести документ
		...
		И я запоминаю заголовок текущего окна как "ЗаголовокОкнаЗнК"
	...
	* Проверить движения	
		...
		Тогда таблица "ОперацииБюджетов" стала равной:
			| 'N' | 'Период'             | 'Вид бюджета'                      | 'Объект планирования' | 'Состояние' | 'Аналитика структуры' | 'Аналитика статей бюджетов'                                   | 'Аналитика номенклатуры' | 'Аналитика потребностей' | 'Аналитика поставки' | 'Аналитика партнеров' | 'Приход расход' | 'Валюта' | 'Валюта лимитирования' | 'Сумма'              | 'Количество' | 'Лимит'    | 'Сумма исполнено' | 'Количество исполнено' | 'Лимит исполнено' | 'Источник версии' | 'Корректировка документа' |
			| '1' | '01.03.2024 0:00:00' | 'Бюджет движения денежных средств' | '$ЗаголовокОкнаЗнК$' | ''          | 'ВА - ЦФО 1 (АХО)'    | 'ВА - Статья 1 (ВА - Здания;  - ;  - ;  - ;  - ;  - ) Расход' | ''                       | ''                       | ''                   | ''                    | 'Расход'        | 'RUB'    | 'RUB'                  | '$ПланыСуммаРезерв$' | ''           | '3 000,00' | ''                | ''                     | ''                | ''                | ''                        |
	
	* Закрыть все окна
		И я закрываю все окна клиентского приложения	

Сценарий: 04.02.3 Проверяем средства к обеспечению в Отчете по лимитам
	* Сформировать отчет по БДДС за 2024
		И Я формирую Отчет по лимитам по виду бюджета 'БДДС' за период с "01.01.2024" по  "31.12.2024" в валюте 'RUB'	
		И Я добавляю отбор в Отчете по лимитам 'ЦФО' 'Равно' 'ВА - ЦФО 1 (АХО)'
        И я нажимаю на кнопку с именем 'СформироватьОтчет'
...
Сценарий: 04.02.4 Создаем Корректировку лимитов по заявке
	...
	* Проверить заполнение таблицы приемника
		Тогда таблица "Требуется" стала равной:
			...
			| '<Не используется>' | '1' | '$ЗаголовокОкнаРезерв$' | ''      | 'Март 2024 г.' | 'Расход'               | 'ВА - ЦФО 1 (АХО)' | 'RUB'    | ''       | 'ВА - Статья 1'  | ''         | ''           | 'ВА - Здания' | '<Не используется>' | '<Не используется>' | '<Не используется>' | '<Не используется>' | '$ПланыСуммаРезерв$' |
	...
	* Проверить движения
		...
		Тогда таблица "ОперацииБюджетов" стала равной:
			...
			| '2' | 'Бюджет движения денежных средств' | 'Расход'        | '1'                       | '$ЗаголовокОкнаРезерв$' | '3 000,00'  | ''                    | ''                | ''          | '1,0000'            | ''                       | ''                | 'ВА - ЦФО 1 (АХО)'    | ''                | ''                   | ''           | '3 000,00'  | 'RUB'    | ''                     | ''                       | 'RUB'                  | '1'                      | 'Нет'                   | '1,0000'             | 'ВА - Статья 1 (ВА - Здания;  - ;  - ;  - ;  - ;  - ) Расход' | ''                        |
			| '3' | 'Бюджет движения денежных средств' | 'Расход'        | '1'                       | ''                      | '-3 000,00' | ''                    | ''                | ''          | '1,0000'            | ''                       | ''                | 'ВА - ЦФО 1 (АХО)'    | ''                | ''                   | ''           | '-3 000,00' | 'RUB'    | ''                     | ''                       | 'RUB'                  | '1'                      | 'Нет'                   | '1,0000'             | 'ВА - Статья 1 (ВА - Здания;  - ;  - ;  - ;  - ;  - ) Расход' | ''                        |
			| '4' | 'Бюджет движения денежных средств' | 'Расход'        | '1'                       | '$ЗаголовокОкнаЗнК$'    | '-3 000,00' | ''                    | ''                | ''          | '1,0000'            | ''                       | ''                | 'ВА - ЦФО 1 (АХО)'    | ''                | ''                   | ''           | '-3 000,00' | 'RUB'    | ''                     | ''                       | 'RUB'                  | '1'                      | 'Нет'                   | '1,0000'             | 'ВА - Статья 1 (ВА - Здания;  - ;  - ;  - ;  - ;  - ) Расход' | ''                        |

	* Закрыть все окна
		И я закрываю все окна клиентского приложения	
	
Сценарий: 04.02.5 Проверяем корректировку лимитов в Отчете по лимитам					
	* Сформировать отчет по БДДС за 2024
		И Я формирую Отчет по лимитам по виду бюджета 'БДДС' за период с "01.01.2024" по  "31.12.2024" в валюте 'RUB'	
		И Я добавляю отбор в Отчете по лимитам 'ЦФО' 'Равно' 'ВА - ЦФО 1 (АХО)'
        И я нажимаю на кнопку с именем 'СформироватьОтчет'
