...
Сценарий: 01.06 Группы аналитик статей
	* Создаем группы раскрытия с разным набором аналитик
		и Я создаю группу аналитики статей 'Группа на 1 вид аналитик' с видами аналитик 'Контрагенты', '', '', '', '', ''
		и Я создаю группу аналитики статей 'Группа на 2 вид аналитик' с видами аналитик 'Контрагенты', 'Номенклатурные группы', '', '', '', ''
		и Я создаю группу аналитики статей 'Группа на 3 вид аналитик' с видами аналитик 'Контрагенты', 'Номенклатурные группы', 'Места поставки', '', '', ''
		и Я создаю группу аналитики статей 'Группа на 4 вид аналитик' с видами аналитик 'Контрагенты', 'Номенклатурные группы', 'Места поставки', 'Подразделения организаций', '', ''
		и Я создаю группу аналитики статей 'Группа на 5 вид аналитик' с видами аналитик 'Контрагенты', 'Номенклатурные группы', 'Места поставки', 'Подразделения организаций', 'Ресурсы', ''
		и Я создаю группу аналитики статей 'Группа на 6 вид аналитик' с видами аналитик 'Контрагенты', 'Номенклатурные группы', 'Места поставки', 'Подразделения организаций', 'Ресурсы', 'Объект расчетов'

		и Я создаю группу аналитики статей 'Контрагенты-НоменклатурныеГруппы' с видами аналитик 'Контрагенты', 'Номенклатурные группы', '', '', '', ''
	
	* Проверяем количество аналитик в группах
		И Я открываю основную форму списка справочника "ГруппыАналитикСтатьи"
		И Я в списке "Группы аналитик статей" по полю "Наименование" ищу элемент 'Группа на 1 вид аналитик' "По точному совпадению"
		И в таблице "Список" я выбираю текущую строку
		И элемент формы с именем "МаксимальныйНомерЗаполненнойАналитики" стал равен '1'
		И Я закрываю окно 'Группа на * вид аналитик (Группа аналитик статьи)'
		
		И Я в списке "Группы аналитик статей" по полю "Наименование" ищу элемент 'Группа на 2 вид аналитик' "По точному совпадению"
		И в таблице "Список" я выбираю текущую строку
		И элемент формы с именем "МаксимальныйНомерЗаполненнойАналитики" стал равен '2'
		И Я закрываю окно 'Группа на * вид аналитик (Группа аналитик статьи)'		

		И Я в списке "Группы аналитик статей" по полю "Наименование" ищу элемент 'Группа на 3 вид аналитик' "По точному совпадению"
		И в таблице "Список" я выбираю текущую строку
		И элемент формы с именем "МаксимальныйНомерЗаполненнойАналитики" стал равен '3'
		И Я закрываю окно 'Группа на * вид аналитик (Группа аналитик статьи)'	

		И Я в списке "Группы аналитик статей" по полю "Наименование" ищу элемент 'Группа на 4 вид аналитик' "По точному совпадению"
		И в таблице "Список" я выбираю текущую строку
		И элемент формы с именем "МаксимальныйНомерЗаполненнойАналитики" стал равен '4'
		И Я закрываю окно 'Группа на * вид аналитик (Группа аналитик статьи)'		

		И Я в списке "Группы аналитик статей" по полю "Наименование" ищу элемент 'Группа на 5 вид аналитик' "По точному совпадению"
		И в таблице "Список" я выбираю текущую строку
		И элемент формы с именем "МаксимальныйНомерЗаполненнойАналитики" стал равен '5'
		И Я закрываю окно 'Группа на * вид аналитик (Группа аналитик статьи)'		

		И Я в списке "Группы аналитик статей" по полю "Наименование" ищу элемент 'Группа на 6 вид аналитик' "По точному совпадению"
		И в таблице "Список" я выбираю текущую строку
		И элемент формы с именем "МаксимальныйНомерЗаполненнойАналитики" стал равен '6'
		И Я закрываю окно 'Группа на * вид аналитик (Группа аналитик статьи)'		

		И Я в списке "Группы аналитик статей" по полю "Наименование" ищу элемент 'Контрагенты-НоменклатурныеГруппы' "По точному совпадению"
		И в таблице "Список" я выбираю текущую строку
		И элемент формы с именем "МаксимальныйНомерЗаполненнойАналитики" стал равен '2'
		И Я закрываю окно 'Контрагенты-НоменклатурныеГруппы (Группа аналитик статьи)'		
		И Я закрываю окно 'Группы аналитик статей'				

Сценарий: 01.07 Статьи БДДС
	* Создаем тестовую группу и добавляем в нее статьи
		И Я создаю группу статей ДДС с именем 'Тестовая группа'
		И Я создаю статью ДДС с именем 'Статья 1' родителем 'Тестовая группа'
		И Я создаю статью ДДС с именем 'Статья 2' родителем 'Тестовая группа'
		И Я создаю статью ДДС с именем 'Статья 3' родителем 'Тестовая группа'
		И Я создаю статью ДДС с именем 'Статья 4' родителем 'Тестовая группа'	

	* Для новых статей определяем тип движения "Приход/Расход"
		И Я для статьи ДДС с именем 'Статья 1' определяю тип движения статьи 'Только расход'
		И Я для статьи ДДС с именем 'Статья 2' определяю тип движения статьи 'Только расход'
		И Я для статьи ДДС с именем 'Статья 3' определяю тип движения статьи 'Приход или расход'
		И Я для статьи ДДС с именем 'Статья 4' определяю тип движения статьи 'Только приход'

	* Назначаем дополнительные аналитики (разрезы) для статьи
		И Я для статьи ДДС с именем 'Статья 1' определяю виды аналитики 'Объект расчетов', '', '', '', '', ''
		И Я для статьи ДДС с именем 'Статья 3' определяю виды аналитики 'Места поставки', '', '', '', '', ''

	* Назначаем дополнительные аналитики (разрезы) группой раскрытия
		И Я для статьи ДДС с именем 'Статья 2' определяю группу раскрытия 'Контрагенты-НоменклатурныеГруппы'

				
Сценарий: 01.08 Статьи БДР
	* Создание группы статей
		И Я создаю группу статей ДиР с именем 'Тестовая группа'
		И Я создаю статью ДиР с именем 'Статья 1' родителем 'Тестовая группа'
		И Я создаю статью ДиР с именем 'Статья 2' родителем 'Тестовая группа'
		И Я создаю статью ДиР с именем 'Статья 3' родителем 'Тестовая группа'
		И Я создаю статью ДиР с именем 'Статья 4' родителем 'Тестовая группа'	

	* Для новых статей определяем тип движения "Доход/Расход"
		И Я для статьи ДиР с именем 'Статья 1' определяю тип движения статьи 'Только расход'
		И Я для статьи ДиР с именем 'Статья 2' определяю тип движения статьи 'Только расход'
		И Я для статьи ДиР с именем 'Статья 3' определяю тип движения статьи 'Доход или расход'
		И Я для статьи ДиР с именем 'Статья 4' определяю тип движения статьи 'Только доход'

	* Назначаем дополнительные аналитики (разрезы) для статьи
		И Я для статьи ДиР с именем 'Статья 1' определяю виды аналитики 'Объект расчетов', '', '', '', '', ''
		И Я для статьи ДиР с именем 'Статья 3' определяю виды аналитики 'Места поставки', '', '', '', '', ''

	* Назначаем дополнительные аналитики (разрезы) группой раскрытия
		И Я для статьи ДиР с именем 'Статья 2' определяю группу раскрытия 'Контрагенты-НоменклатурныеГруппы'